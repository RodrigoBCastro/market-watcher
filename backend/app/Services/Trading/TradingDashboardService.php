...
class TradingDashboardService
{
    ...
    public function build(int $userId): array
    {
        ...
        $callsByStatus = $this->tradeCallRepository->countByStatus();

        $topCalls = $this->tradeCallRepository->listTopRanked(8)
            ->map(static fn ($call): array => [
                'id'              => (int) $call->id,
        ...
        $alertsUnread = $this->tradingAlertRepository->countUnread($userId);
        $universes    = $this->marketUniverseService->summary();

        $pnlOpen = array_sum(array_map(
            static fn (array $item): float => (float) ($item['unrealized_pnl'] ?? 0.0),
            $openPositions,
        ));
