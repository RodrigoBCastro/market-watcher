...
class SetupMetricsService implements ProbabilisticEngineInterface
{
    ...
    {
        $grouped = TradeOutcome::query()->orderBy('created_at')->get()->groupBy('setup_code');
        ...
        foreach (SetupCode::cases() as $setupCode) {
            ...
            $model = SetupMetric::query()->updateOrCreate([
    ...
    {
        $model = SetupMetric::query()->where('setup_code', strtoupper($setupCode))->first();
    ...
    public function listSetupMetrics(): array
    {
        return SetupMetric::query()
            ...
            ->get()
            ->map(fn (SetupMetric $row): array => $this->toDto($row)->toArray())
            ->all();
    }
    ...
    {
        $outcomes = TradeOutcome::query()->orderBy('created_at')->get();
    ...
    private function alerts(array $outcomes): array
    {
        ...
        $minHistory = (int) config('market.calls.min_history', 8);
        $deteriorating = SetupMetric::query()
            ->where('total_trades', '>=', $minHistory)
            ->where(function ($query): void {
                $query->where('expectancy', '<=', 0)
                    ->orWhere('winrate', '<=', 50);
